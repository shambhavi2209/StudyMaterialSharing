...
class study:
    ...
    def __init__(self, root):
        ...
        self.bg_fit = Label(self.root, image=self.bg).place(
            x=0, y=0, relwidth=1, relheight=1)
        title = Label(self.root, text="Study Material application", font="comicsansms 35 bold",
                      fg="white", bg="#2f2641", relief=SUNKEN, borderwidth=10)
        title.pack(pady=90, ipadx=5, ipady=5)

        # Account Frame
        account_frame = Frame(self.root, bg="white",
                              relief=RAISED, borderwidth=30)
        account_frame.place(x=190, y=200, height=440, width=500)

        # Login page
        login_title = Label(account_frame, text="Login", font="Impact 35 bold",
                            fg="#2f2641", bg="white").place(x=90, y=30)
        span = Label(account_frame, text="Login to access",
                     font="comicsansms 10 bold", fg="#d25d17", bg="white").place(x=95, y=100)
        self.name = StringVar()
        self.password = StringVar()

        username = Label(account_frame, text="Username..",
                         font="comicsansms 15 bold", fg="#4d4d4d", bg="white").place(x=40, y=140)
        self.user_entry = Entry(account_frame, textvariable=self.name, font=(
            "times new roman", 15), bg="lightgray", relief=SUNKEN, borderwidth=6)
        self.user_entry.place(x=45, y=173, width=350, height=35)

        password = Label(account_frame, text="Password..",
                         font="comicsansms 15 bold", fg="#4d4d4d", bg="white").place(x=40, y=215)
        self.user_pass = Entry(account_frame, textvariable=self.password, show="*", font=(
            "times new roman", 15), bg="lightgray", relief=SUNKEN, borderwidth=6)
        self.user_pass.place(x=45, y=247, width=350, height=35)

        new_user = Button(account_frame, text="New User?", cursor="hand2", command=self.signup,
                          bg="white", fg="#d25d17", font=("times new roman", 12), relief=FLAT).place(x=230, y=287)
        Log_button = Button(account_frame, command=self.login, cursor="hand2", text="Login", fg="white",
                            bg="#d25d17", font=("times new roman", 20), relief=GROOVE).place(x=80, y=290, height=40)

    def signup(self):
        self.root = root
        # Same as Tk() but it is used for creating window over window with all functions same.
        self.top = Toplevel()
        ...
        signup_frame = Frame(self.top, bg="white",
                             relief=RAISED, borderwidth=30)
    ...
    # This below funtion calls when sign_button is clicked
    def sign(self):
        if self.sign_name.get() == "" or self.sign_password.get() == "":
            messagebox.showerror("Error", "All fields are mandatory")
        else:
            conn = mysql.connector.connect(
                host="localhost", user="root", password="ashok1965", database="student")
            cur = conn.cursor()
            cur.execute("insert into signup values(%s,%s)", (
                self.sign_name.get(),
                self.sign_password.get()
            ))
        conn.commit()
        conn.close()
    ...
    def login(self):
        if self.name.get() == "" or self.password.get() == "":
            ...
            conn = mysql.connector.connect(
                host="localhost", user="root", password="ashok1965", database="student")
            cur = conn.cursor()
            cur.execute("select * from signup where username=%s and password=%s", (
                self.name.get(),
                self.password.get()
            ))
    ...
    def mainwork(self):
        ...
        self.bg_fit = Label(self.newroot, image=self.bg).place(
            x=0, y=0, relwidth=1, relheight=1)
        ...
        c1 = Canvas(self.newroot, width=300, height=300,
                    relief=RAISED, borderwidth=20, bg="grey")
        c1.pack()
        ...
        b = Button(c1, image=image1, compound=LEFT, relief=FLAT,
                   bg="grey", command=lambda: self.common("Python"))
        b.pack(side=LEFT, padx=23, pady=23)

        b = Button(c1, image=image2, compound=LEFT, relief=FLAT,
                   bg="grey", command=lambda: self.common("Maths"))
        b.pack(side=LEFT, padx=23, pady=23)

        b = Button(c1, image=image3, compound=LEFT, relief=FLAT,
                   bg="grey", command=lambda: self.common("C++"))
        b.pack(side=LEFT, padx=23, pady=23)

        b = Button(c1, image=image4, compound=LEFT, relief=FLAT,
                   bg="grey", command=lambda: self.common("DSA"))
        b.pack(side=LEFT, padx=23, pady=23)

        # Creating second canvas for below 4 subjects.
        c2 = Canvas(self.newroot, width=300, height=300,
                    relief=RAISED, borderwidth=20, bg="grey")
        c2.pack(pady=10)
        ...
        b2 = Button(c2, image=image21, compound=LEFT, relief=FLAT,
                    bg="grey", command=lambda: self.common("DBMS"))
        b2.pack(side=LEFT, padx=23, pady=23)

        b2 = Button(c2, image=image22, compound=LEFT, relief=FLAT,
                    bg="grey", command=lambda: self.common("English"))
        b2.pack(side=LEFT, padx=23, pady=23)

        b2 = Button(c2, image=image23, compound=LEFT, relief=FLAT,
                    bg="grey", command=lambda: self.common("Java"))
        b2.pack(side=LEFT, padx=23, pady=23)

        b2 = Button(c2, image=image24, compound=LEFT, relief=FLAT,
                    bg="grey", command=lambda: self.common("Software_engg"))
        b2.pack(side=LEFT, padx=23, pady=23)
        mainloop()
    ...
    def common(self, x):
        ...
        self.back_fit = Label(second_frame, image=self.back).place(
            x=0, y=0, relwidth=1, relheight=1)

        # After this above execution second_frame will act as a new frame as root for further execution

        title = Label(second_frame, text=x, font="Impact 35 bold",
                      fg="#2f2641", bg="white")
        title.pack(pady=30, ipadx=5, ipady=5)
        ...
        # This excel file contains the titles of that specific book.
        books_name = load_workbook(f"data/booksname/{x}.xlsx")
        ...
        for i in range(len(files)):
            ...
            list1[i] = Canvas(second_frame, width=320, height=300,
                              relief=RAISED, borderwidth=20, bg="#add5ff")
            list1[i].pack(ipadx=100, padx=120)

            self.image1[i] = ImageTk.PhotoImage(
                file=f"images/books/{x}/{i+1}.png")
            self.down = ImageTk.PhotoImage(file="images/down.png")
            self.view = ImageTk.PhotoImage(file="images/view.png")

            Button(list1[i], image=self.image1[i]).pack(
                side=LEFT, pady=23, padx=15)

            Label(list1[i], text=f"{book.value}", font="comicsansms 18 bold", fg="black",
                  bg="#6e81fa", relief=SUNKEN, borderwidth=10).pack(side=TOP, pady=60)

            btn.append(Button(list1[i], text=files[i], relief=RAISED, borderwidth=6, command=lambda c=i: os.startfile(
                f"data\\books\\{x}\\{btn[c].cget('text')}.pdf")))
            btn[i].pack(side=BOTTOM, padx=10, pady=40)

            btn2.append(Button(list1[i], text=files2[i], relief=RAISED, borderwidth=6,
                        command=lambda c=i: self.speak_pdf(x, btn[c].cget('text'))))
            btn2[i].pack(side=BOTTOM, padx=23)

    def speak_pdf(self, x, y):
        # print(y)
        a = PyPDF2.PdfFileReader(f"data\\booksintro\\{x}\\{y}.pdf")
        str = "Hello Reader! The Small intro of this pdf is ! " + \
            a.getPage(0).extractText(
            ) + "!!The More Details you will be find in this book, Do check it out."
        self.speak(str)
